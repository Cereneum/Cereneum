...
contract CereneumImplementation is CereneumData
{
  ...
  function UpdateDailyData() public
    ...
    for(m_nLastUpdatedDay; DaysSinceLaunch() > m_nLastUpdatedDay; m_nLastUpdatedDay++)
      ...
      if(m_nLastUpdatedDay < 350)
			{
        nUnclaimedCoins = m_nMaxRedeemable.sub(m_nTotalRedeemed);
        nUnclaimedCoins = nUnclaimedCoins.div(350); //50 weeks

        nPayoutRound = nPayoutRound.add(nUnclaimedCoins);

        nPayoutRound = nPayoutRound.add(
          //Frenzy bonus 0-50% based on total users claiming
          nPayoutRound.mul(m_nRedeemedCount).div(m_nUTXOCountAtSnapshot).div(2)
        ).add(
          //Prosperous bonus 0-50% based on size of claims
          nPayoutRound.mul(m_nTotalRedeemed).div(m_nAdjustedMaxRedeemable).div(2)
        );

        //Pay frenzy and Prosperous bonuses to genesis address
        _mint(m_genesis, nPayoutRound.mul(m_nRedeemedCount).div(m_nUTXOCountAtSnapshot).div(2)); // Frenzy
        _mint(m_genesis, nPayoutRound.mul(m_nTotalRedeemed).div(m_nAdjustedMaxRedeemable).div(2)); // Prosperous
      }
			else
			{
  ...
  /// @return Correctly formatted message for bitcoin signing
    ...
    else  //Otherwise LTC
    {
      ...
      prefix = "\x19Litecoin Signed Message:\n\x3CClaim_Cereneum_to_0x";
		...
		{
    	message[offset++] = prefix[i];
    }

		for(uint i = 0; i < 20; i++)
		{
      message[offset++] = hexDigits[uint256(uint8(addressBytes[i] >> 4))];
      message[offset++] = hexDigits[uint256(uint8(addressBytes[i] & 0x0f))];
    }
  ...
  function CanClaimUTXOHash(
    ...
    return(
			(m_claimedAddressesMap[uint8(a_nWhichChain)][a_hMerkleLeafHash] == false) && VerifyProof(a_hMerkleTreeBranches, a_hMerkleLeafHash, a_nWhichChain)
    );
  }
  ...
  function CanClaim(
    ...
    return a_nAmount.mul(nMaxDays.sub(DaysSinceLaunch())).div(nMaxDays);
	}
  ...
  /// @param a_nWhichChain Which blockchain is claiming, 0=BTC, 1=BCH, 2=BSV, 3=ETH, 4=LTC
  /// @return Claim amount and bonuses
  function GetRedeemAmount(uint256 a_nAmount, BlockchainType a_nWhichChain) public view returns (uint256, uint256)
	{
    if(a_nWhichChain != BlockchainType.Bitcoin)
    {
      ...
      --nWhichChain;
			...
			uint256 nScalar = 1000000000000000;
  ...
  function ValidateOwnership(
    ...
    //TODO: bitcoin/eth raw addresses instead of keccak hashes will save 12bytes per entry
    //Save the UTXO as redeemed in the global map
    m_claimedAddressesMap[uint8(a_nWhichChain)][hMerkleLeafHash] = true;
  }
  ...
  function Claim(
    ...
    (uint256 nTokensRedeemed, uint256 nBonuses) = GetRedeemAmount(a_nAmount, a_nWhichChain);
    ...
    bool bWasReferred = a_referrer != address(0);

    if(bWasReferred)
		{
			//Grant 10% bonus token to the person being referred
			_mint(address(this), nTokensRedeemed.div(10));
      ...
      _mint(m_genesis, nTokensRedeemed.mul(1000000).div(3333333));
    }
  ...
  /// @param a_nStakeIndex index of stake to compound interest for
	/// @param a_address TEMPORARY FOR TESTING
  function CompoundInterest(
		uint256 a_nStakeIndex,
		address a_address //TEMPORARY FOR TESTING
	) external
	{
		//TODO: Decide if compounding once every minimum stake amount is better than daily
		address MSG_SENDER = a_address;	//TODO: Replace with msg.sender
    ...
    StakeStruct storage rStake = m_staked[MSG_SENDER][a_nStakeIndex];
  ...
  function StartStake(
    ...
    uint256 a_nDays,
		...
		address a_address	//TODO: address is temporary for testing
  ) external
	{
		//TEMPORARY FOR TESTING. REMOVE BEFORE LAUNCH
		address MSG_SENDER = a_address;
    //Verify account has enough tokens
    require(balanceOf(MSG_SENDER) >= a_nAmount, "Not enough funds for stake.");
    ...
    _transfer(MSG_SENDER, address(this), a_nAmount);
  }
